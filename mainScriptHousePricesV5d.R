...
build_model <- function() {
  ...
  layer_dense (units              = 1) %>% #=> Common for Binary
    # (4) Compile Model-----------------------------------------------------
  compile (optimizer = optimizer_rmsprop(lr = 0.001, rho = 0.9, epsilon = 0.001, decay = 0.001,
                                         clipnorm = NULL, clipvalue = NULL)
           , #=> Most Popular for Optimization Algo.
...
for (i in 1:k) {
  ...
  history <- model %>% fit(partial_train_data, partial_train_targets,
                ...
                epochs = num_epochs, batch_size = 4, verbose = 1, shuffle = TRUE)
...
model %>% fit(as.matrix(x_train_tbl), y_train_vec,
              epochs = 120, batch_size=4, verbose=1, shuffle = TRUE)
