...
build_model <- function() {
  model <- keras_model_sequential() %>%
    # (1) 1st Hidden Layer-------------------------------------------------
    layer_conv_1d (filters        = 32, #=> Num Of Nodes
                 kernel_size      = 0,
                 activation       = "relu",    
                 input_shape      = c(0, ncol(x_train_tbl))) %>% 
    layer_max_pooling_1d(pool_size = 1) %>%
    #layer_dropout (rate = 0.15) %>%  #=> Dropout Below 10%: Prevent overfitting
    # (2) 2nd Hidden Layer-------------------------------------------------
    layer_conv_1d (filters        = 32, #=> Num Of Nodes
                 kernel_size      = 0,
                 activation       = "relu") %>%
...
model %>% fit(as.matrix(x_train_tbl), y_train_vec,
              epochs = 37, batch_size=2, verbose=1)
